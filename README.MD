...
| 并发队列控制 | 每账号 in-flight 上限 + 等待队列，O(1) cursor 轮询，动态计算建议并发值 |
| DeepSeek PoW | 纯 Go 高性能实现（DeepSeekHashV1），自适应多核并行，AVX-512 / arm64 NEON 加速 |
| 连接管理 | HTTP Transport 调优，空闲连接自动回收，ResponseHeaderTimeout 防挂死，双层 LimitReader 防 OOM |
...
| default | `deepseek-v4-flash-rp` | 默认开启，可由请求参数控制；RP 优化；不注入工具提示词 | ❌ |
| default | `deepseek-v4-flash-nothinking-rp` | 永久关闭；RP 优化；不注入工具提示词 | ❌ |
| expert | `deepseek-v4-pro` | 默认开启，可由请求参数控制 | ❌ |
| expert | `deepseek-v4-pro-nothinking` | 永久关闭，不受请求参数影响 | ❌ |
| expert | `deepseek-v4-pro-rp` | 默认开启，可由请求参数控制；RP 优化；不注入工具提示词 | ❌ |
| expert | `deepseek-v4-pro-nothinking-rp` | 永久关闭；RP 优化；不注入工具提示词 | ❌ |
...
除原生模型外，也支持常见 alias 输入（如 `gpt-4.1`、`gpt-5`、`gpt-5-codex`、`o3`、`claude-*`、`gemini-*` 等），但 `/v1/models` 返回的是规范化后的 DeepSeek 原生模型 ID。OpenAI-compatible Chat / Responses 中，若 alias 名本身追加 `-nothinking` 后缀，也会映射到对应的强制关思考模型；追加 `-rp` 会映射到 reduced-prompt / roleplay 变体。`-rp` 会跳过工具 schema 与 DSML 工具调用规则提示词注入；current input file 开启时会始终上传完整 `DS2API_HISTORY.txt` 上下文，并使用精简 live prompt：边界句、最新用户输入、RP continuation 约束、可选 thinking injection。Claude-compatible / Gemini-compatible 原生入口不支持 `-rp`。`thinking_enabled`、`model_type` 和工具输出解析仍保持对应基础模型行为。完整 alias 行为以 [API.md](API.md#模型-alias-解析策略) 和 `config.example.json` 为准。
...
累计请求数默认写入 `./data/runtime_stats.json`，重建或重启容器后管理台统计仍会保留。
...
- `runtime`：账号并发、队列、PoW 计算并发与 token 刷新策略，可通过 Admin Settings 热更新。
- `auto_delete.mode`：请求结束后的远端会话清理策略，支持 `none` / `single` / `all`。
- `current_input_file`：全局生效的上下文拆分上传策略；默认开启且普通模型阈值为 `12000` 字符，触发时将完整上下文合并上传为 `DS2API_HISTORY.txt` 上下文文件；`-rp` 模型仍在开启时始终上传。
