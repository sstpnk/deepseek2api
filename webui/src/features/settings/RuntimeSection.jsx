    ...
    return (
        <div className="bg-card border border-border rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <label className="text-sm space-y-2">
                    <span className="text-muted-foreground">{t('settings.powMaxConcurrency')}</span>
                    <input
                        type="number"
                        min={1}
                        max={256}
                        step={1}
                        value={form.runtime.pow_max_concurrency}
                        onChange={(e) => setForm((prev) => ({
                            ...prev,
                            runtime: { ...prev.runtime, pow_max_concurrency: Number(e.target.value || 1) },
                        }))}
                        className="w-full bg-background border border-border rounded-lg px-3 py-2"
                    />
                    <p className="text-xs text-muted-foreground">{t('settings.powMaxConcurrencyHelp')}</p>
                </label>
            </div>
        </div>
    )
