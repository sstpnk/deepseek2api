...
- **Proxy/account health protection**: transport failures put the active proxy into exponentially increasing cooldown, and retries avoid proxies that already failed within the same request. OpenAI Chat / Responses empty-output synthetic retries first avoid the current proxy; if the same account still returns `upstream_empty_output` three times within a rolling 20-minute window, it enters exponentially increasing cooldown, but empty-output account cooldowns are guarded by a global budget to avoid marking most accounts unavailable during upstream or proxy-wide incidents. Visible successful output gradually decays both the empty-output counter and account cooldown level. Clear account-auth failures that cannot be fixed by refresh also put the account into exponentially increasing cooldown and temporarily skip it in the account pool. Admin account-pool status includes usable account count plus `cooling_down` / `cooling_down_accounts_total`.
