...
func TestMessagesPrepareUsesTurnSuffixes(t *testing.T) {
	...
	if !strings.HasSuffix(got, "<｜Assistant｜>Answer<｜end▁of▁sentence｜>") {
		t.Fatalf("expected trailing assistant block with end-of-sentence marker, got %q", got)
	}
...
func TestMessagesPrepareClosesAssistantTurns(t *testing.T) {
	messages := []map[string]any{
	...
	if !strings.HasSuffix(got, "<｜Assistant｜>answer<｜end▁of▁sentence｜>") {
		t.Fatalf("final assistant should be closed, got %q", got)
	}
	if strings.Count(got, "<｜end▁of▁sentence｜>") != 2 {
		t.Fatalf("expected two end-of-sentence markers, got %q", got)
	}
}
