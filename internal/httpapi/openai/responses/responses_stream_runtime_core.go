...
func (s *responsesStreamRuntime) finalize(finishReason string, deferEmptyOutput bool) bool {
	...
	if outcome.ShouldFail {
		...
		if promoted, ok := promoteThinkingWhenTextEmpty(turn.Text, turn.Thinking, turn.ContentFilter); ok {
			...
			turn.Error = nil
			turn.Usage = assistantturn.BuildUsage(s.model, s.finalPrompt, turn.Thinking, turn.Text, s.refFileTokens)
			outcome = assistantturn.FinalizeTurn(turn, assistantturn.FinalizeOptions{
				AlreadyEmittedToolCalls: s.toolCallsEmitted || s.toolCallsDoneEmitted,
			})
		} else {
			s.failResponse(outcome.Error.Status, outcome.Error.Message, outcome.Error.Code)
			return true
		}
	}
